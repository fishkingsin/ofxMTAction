...
void ofxMtActionsObject::update() {
	...
	if(touches.size()>0) {
		...
		ofVec2f deltaTotal = ofVec2f(0,0);
		...
		if(deltaCount>0) {
			ofVec2f delta = deltaTotal/deltaCount;
	...
	if(!moved) {
		transform(ofVec2f(0,0), 0, 1); // damping
	}
...
//--------------------------------------------------------------
void ofxMtActionsObject::transform(ofVec2f vec, float angle, float _scale) {
